...
\newcommand\BLedit[1]{{\color{blue}{#1}}}
...
then be applied to the more distant stars\BLedit{, thanks to the simple physical relations connecting the measurements (apparent magnitudes, parallaxes) to some of the physical parameters of interest (absolute magnitudes, distances).}
...
&=&  \ \mathcal{N}(y_n \given m\,x_n + b, t^2 + \sigma_n^2)
\label{eq:toyLike}
\end{eqnarray}

where we have marginalized over the true sample $y_{\true, n}$.} The resulting likelihood is a Gaussian because the noise model is a Gaussian. \BLcomment{should all these equations have $x_n$ on the right hand side? I don't have a strong opinion about that.} The likelihood of the full data set is then the product of the individual likelihoods
...
\item[stationarity] We make an assumption of \emph{stationarity}; that is, that the
  ...
  work. \BLcomment{Isn't that assumption made conditionally on color and magnitude in the CMD? In other words, isn't the assumption that there are low- and high-SNR stars (and that those are statistically drawn from the same distribution of physical parameters) in any color--magnitude cell?}
...
\item[noise model] We assume that the \tgas\ parallax uncertainties dominate the
  ...
  de-noising, under-estimation of measurement noise is conservative, and leads to under-deconvolution. \BLedit{In other words, the features of the inferred \cmd\ will typically be broader and more conservatively estimated than than if we over-estimated the noise.} \BLcomment{please rephrase if this is unclear. I think it is worth clarifying this "under-deconvolution". I noticed people don't typically understand this when they hear/read it, and it is an important aspect of this paper.}
...
\BLcomment{Is it the right place to comment that this whole approach also assumes that 1) the selection function is totally known (and neglected here), 2) all measurements and their errors are correct (unbiased and statistically representative)? This could also go in the discussion, but I personally see those as fundamental assumptions.}
...
\BLcomment{This whole section is highlighted because I have moved paragraphs around and also added small clauses here and there. I've tried to improve the logical motivation and presentation using the (great) paragraphs you wrote. Feel free to revert if you don't like it.}\BLedit{
...
parallax, and $Z$ is a normalization constant \BLedit{(the model evidence of marginalized likelihood)}.
...
We include this distance information in the inference of $\varpi_{\true}$ by using the \cmd\ as our prior.
Specifically, we use the \cmd\ generated from the data themselves as our prior.

In this context, equation~\ref{eq:bayes} is significantly complexified.
...
We now describe a particularity of our method, which will allow us to conservatively deconvolve the data into a \cmd\ prior without requiring a full hierarchical inference framework (which was explored in \citealt{leistedtHogg2017}).
...
Before we describe how this can be turned into a posterior belief on the true parallax, we turn our attention to the prior.
}

\subsection{Generating the Prior}

To generate our prior $p(\mathbf{Y}_n)$, we fit the data in the slightly transformed \cmd\ space, using Equation \ref{eq:transform} instead of absolute magnitude. We fit this transformed \cmd\ using \xd\ which deconvolves the data $\{ \mathbf{Y}_n \}$ with its uncertainties $\{ V_n\}$, which are heterogeneous measurement uncertainties. It generates an estimate of the underlying distribution from which the uncertain data were drawn from. \xd\ generates a representation of the distribution as a $K$-mixture of Gaussians. In other words, the prior reads
\BLedit{
\begin{equation}
...
\end{equation}
where $\mathbf{\mu}_k$ and $V_k$ are a two dimensional vector and matrix, respectively, which correspond to the location and covariance of the $k$ component, which is a Gaussian in our transformed \cmd\ space. The relative amplitudes between the components are described by the scalars $\{ \mathrm{amp}_k \}$. 
}
Given the data $\{ \mathbf{Y}_n, V_n\}$, \xd\ maximizes the marginalized likelihood of the data, marginalizing over the true data, \textit{i.e.}, finds the coefficients
\begin{eqnarray}
\alpha &=& \left\{\mathrm{amp}_k, \mathbf{\mu}_k, V_k\right\}_{k=1}^K \; k \in \mathrm{Gaussians} \nonumber\\
...
\BLedit{To infer $\varpi_{\true}$, we first do the inference in the 2D space of the \cmd\ prior $p(\mathbf{Y} \given \alpha)$, using Equation \ref{eq:posterior}.} We then need to project the 2D posterior into the 1D transformed absolute magnitude space, and do a little algebra to solve for $\varpi_{\true}$
...
\BLcomment{Also add the statistics about the parallax uncertainty shrinkage. Or put in a separate subsection? In fact, I now realize some of the figures are not discussed in the text. }
