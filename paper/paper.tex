...
Our posterior photometric parallax estimates are more precise than the \tgas\ catalog entries by a median factor of 1.2, and a 75, 90, and 99 percentile factor of 1.6, 2.3, and 4.9, respectively; and more precise than previous examples of Bayesian distance estimates by a median factor of 1.2, and a 75, 90 and 99 percentile factor of 1.5, 1.6, and 2.1, respectively.
...
\item[noise model] We assume that the \tgas\ parallax uncertainties dominate the
  ...
  correctly known variances. In detail, we assume that the parallax uncertainty estimates, the parallax{\_}error entry in the \gaia\ catalogue, are correct.
...
parallax, and $Z$ is a normalization constant \BLedit{(the model evidence, or marginalized likelihood)}.
...
To generate our prior $p(\mathbf{Y}_n)$, we fit our data in the slightly transformed \cmd\ space using \xd, which deconvolves the data $\{ \mathbf{Y}_n \}$ with its uncertainties $\{ V_n\}$, which are Gaussian, heterogeneous measurement uncertainties. It generates an estimate of the underlying distribution from which the uncertain data were drawn from, modeled as a $K$-mixture of Gaussians:
\BLedit{
\begin{equation}
...
\BLedit{To infer $\varpi_{\true}$ once the \cmd\ prior is constructed, we first do the inference in the 2D space of the \cmd\ prior $p(\mathbf{Y} \given \alpha)$, using Equation \ref{eq:posterior}.} We then project the 2D posterior into the 1D transformed absolute magnitude space, and do a little algebra to solve for $\varpi_{\true}$
...
TODO: talk about crossvalidation?
...
\caption{\BLedit{The \cmd\ prior, modeled as a Gaussian mixture inferred by running XD on all of the data. The left panel shows a sampling of the prior,  with the blue lines showing the $1\sigma$ and $2\sigma$ contours. The right panel shows the 1-$\sigma$ contours of the individual components. The latter are Gaussian in the transformed magnitude space to make the XD inference tractable. Thus, they appear non-Gaussian in the physical color--magnitude space.}}
\label{fig:prior}
\end{figure}

\subsection{prior}
Using \xd\ and the MML method described in the methods section, we generate our prior shown in Figure \ref{fig:prior}.
...
\BLcomment{Figure \ref{fig:prior} needs to be discussed here or in a separate subsection. I also find that it is the right place to discuss its features. I find it strange to discuss the posterior parallaxes without describing the prior.}
...
\section{Conclusion}
If you want a conclusion, re-read the abstract
