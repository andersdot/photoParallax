...
The $1\sigma$ contours as well as (noise free) samples from the toy model distribution are shown in \figurename~\ref{fig:toy}, upper-left panel in red.
...
The MMLE gives us the best fit parameters $\hat{m}$, $\hat{b}$, and $\hat{t}$. In \figurename~\ref{fig:toy}, lower-left panel, the true functions $y_{\true} = m_{\true}\,x + b_{\true} \pm t_{\true}$ are shown as the red lines, and the best fit MMLE functions $\hat{y}_{\true} = \hat{m}\, x + \hat{b} \pm \hat{t}$ are shown as the green lines. Now that we have an estimate of the true underlying distribution $\hat{y}_{\true}$, we use this as a prior to infer a posterior belief of the true values for each datum $p(y_{\true,n} \given y_n, \tilde{\sigma}_n^2)$.
...
For the remainder of the paper, we will also maintain the color scheme presented here (where possible): truth will be red, data will be black, prior information will be green, and posterior information will be blue.
...
\item[noise model] We assume that the \tgas\ parallax uncertainties dominate the
  ...
  de-noising data, under estimating measurement noise is conservative, and leads to under-deconvolution. In other words, the features of the inferred \cmd\ will typically be broader and more conservatively estimated than if we over-estimated the noise.
...
Examples 1, 3, and 4 are not very effected by the prior (green), where good data is good data.
...
The remaining panels show the posterior PDF in blue, the prior PDF as shaded in green, and likelihood of the data in black. 
...
The vertical red dotted lines bracket the previously measured parallaxes of, or distances to, the cluster.
...
The upper right panel shows the \tgas\ parallax likelihoods in black, with the ``true" parallax to M67, determined by previous methods, bracketed by the red dotted lines.
...
\added{As a second test of accuracy of these photometric parallax inferences, we compared to the inferred distances to the stars from asteroseismology.  Asteroseimology can make much more precise distance, and therefore parallax, estimates to stars. There are about 2200 stars that overlap our data set here and the asteroseimic data from Huber at all. We compared the estimates in the parallax space (since the distance estimates are more precise, this is a safer/better comparison space) and show that our parallax estimates are unbiased relative to asteroseismic estimates, and are also more similar to the asteroseimic estimates than tgas parallax likelihoods. }
\begin{figure}
\centering
  \includegraphics[width=\textwidth]{photoVSseismic.pdf}
\caption{Proof of concept: parallax estimate comparisons.}
