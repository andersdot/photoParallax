...
\added{The long-term complete distance information coming from \gaia\ will
...
\added{It also handles incomplete data, in the sense of there being missing
...
Way.
The \added{fundamental and critical} assumption made in this work is that for every star
...
\item[noise model] We assume that the \tgas\ parallax uncertainties dominate the
  ...
  de-noising data, under estimating measurement noise is conservative, and leads to under-deconvolution. In other words, the features of the inferred \cmd\ will typically be broader and more conservatively estimated than if we over-estimated the noise. \added{We need to make these assumptions, that the noise model is Gaussian for both the parallaxes and the colors (including dust corrections), to be able to use the tool \xd, which requires Gaussian uncertainties, to deconvolve the data with it's noise for generating the prior. We also ignore the star to star covariances because they are not available in the \tgas\ dataset.}
...
The tight correlation of the \cmd\ benefits our inference because the luminosity or absolute magnitude of a star $M$, when compared with its brightness or apparent magnitude $m$, contains \added{parallax information $\varpi$ via the fundamental relation $m=M+10-5\log(\varpi[\mathrm{mas}])$.}
...
\added{There is also a \tgas\-\tmass\ cross-match as part of the official \gaia\ product, but this project began before that catalogue was available.}
...
\caption{\added{Five example posterior PDFs over parallax using our method for more rare objects.
